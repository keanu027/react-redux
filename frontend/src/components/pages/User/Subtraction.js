...
import {
  ...
  UpdateUserSub,
  DeleteUserSub,
} from '../../action/subtraction';

import {
  HandleClose,
  ShowMessage,
  fetchSubtractRequest,
  fetchSubtractFailure,
  FetchSubtractListSuccess,
} from '../../../redux';

export default function Subtraction(props) {
  const dispatch = useDispatch();

  const loading = useSelector((state) => state.subtraction.loading);
  const data = useSelector((state) => state.subtraction.data);
  const grandtotal = useSelector((state) => state.subtraction.grandtotal);
  // Toast Notif
  ...
  useEffect(() => {
    ...
    if (isSubscribed) {
      SubtractionList(localStorage.getItem('token'))
        .then((res) => {
          console.log('Loading Subtraction Data...');
          ...
          dispatch(FetchSubtractListSuccess(res.data, total));

          dispatch(fetchSubtractRequest());
        })
        .catch((err) => {
          console.log(err);
          dispatch(
            fetchSubtractFailure(err.response.data.error, true, 'warning')
          );
        });
    }

    return () => {
      console.log('unmounting Subtraction Page');
      isSubscribed = false;
    };
  }, [dispatch]);
  ...
  function AddRow(arg) {
    ...
    Register(localStorage.getItem('token'), arg.firstnum, arg.secondnum)
      .then((res) => {
        ...
        dispatch(FetchSubtractListSuccess(newdata, count));
      })
      .catch((err) => {
        ...
        dispatch(fetchSubtractFailure(errormessage, true, 'warning'));
      });
  }
  ...
  function DeleteRow(arg) {
    console.log(arg);
    let count = 0;
    DeleteUserSub(localStorage.getItem('token'), arg.id)
      .then((res) => {
